...
const SideMenu = () => {
  ...
  return (
    ...
    // </div>
    <aside className={`lg:w-[232px] w-full h-screen fixed top-0 lg:sticky bg-white z-[999] ${isOpen ? 'w-[60px]' : ''}`}>
      <nav className="h-screen flex flex-col bg-white border-r shadow-sm ">
        <div className="p-4 pb-2 flex gap-[8px] text-[16px] items-center">
          <div className={`lg:flex hidden flex-row items-center ${isOpen ? 'block' : 'none'}`}>
            <img src={logo} alt="logo" className="w-[20px] h-[20px]"/><span>TimeSync</span>
          </div>
        ...
        <Link to='/dashboard'>
          ...
          <span className={`hidden lg:inline ${isOpen ? 'lg:visible' : ''}`}>Dashboard</span> <img src={dashboard} alt="dashboard"/>
          </li>
          </Link> 
          ...
          <span className={`hidden lg:inline ${isOpen ? 'lg:visible' : ''}`}>Teams </span>
          </li>
        ...
        <Link  to="time-tracker">
          <li className="relative flex items-center gap-[8px] text-[12px] text-[#7B7C7C] py-2 px-3 my-1 font-medium rounded-md cursor-pointer lg:hover:w-[184px] hover:w-[32px] lg:hover:h-[40px] hover:h-[36px] hover:bg-[#E3EFFC] hover:border-[#B6D8FF] hover:rounded-[4px]">
           ...
           <span className={`hidden lg:inline ${isOpen ? 'lg:visible' : ''}`}>Time tracker</span>
          </li>
          </Link>
          <li className="relative flex items-center gap-[8px] text-[12px] text-[#7B7C7C] py-2 px-3 my-1 font-medium rounded-md cursor-pointer lg:hover:w-[184px] hover:w-[32px] lg:hover:h-[40px] hover:h-[36px] hover:bg-[#E3EFFC] hover:border-[#B6D8FF] hover:rounded-[4px]">
          <img src={projects} alt="projects"/> 
          <span className={`hidden lg:inline ${isOpen ? 'lg:visible' : ''}`}>Projects </span>
          </li>
          <li className="relative flex items-center gap-[8px] text-[12px] text-[#7B7C7C] py-2 px-3 my-1 font-medium rounded-md cursor-pointer lg:hover:w-[184px] hover:w-[32px] lg:hover:h-[40px] hover:h-[36px] hover:bg-[#E3EFFC] hover:border-[#B6D8FF] hover:rounded-[4px]">
          <img src={analytics} alt="analytics"/> 
          <span className={`hidden lg:inline ${isOpen ? 'lg:visible' : ''}`}>Analytics and report</span>
          </li>
        </ul>
        <ul className="flex-1 px-3 pt-[90px]">
          <li className="relative flex items-center gap-[8px] text-[12px] text-[#7B7C7C] py-2 px-3 my-1 font-medium rounded-md cursor-pointer lg:hover:w-[184px] hover:w-[32px] lg:hover:h-[40px] hover:h-[36px]  hover:bg-[#E3EFFC] hover:border-[#B6D8FF] hover:rounded-[4px]">
          <img src={settings} alt="settings"/> 
          <span className={`hidden lg:inline ${isOpen ? 'lg:visible' : ''}`}>Settings</span> 
          </li>
        </ul>
        <div className="flex flex-col p-3 items-center text-[12px]">
          <img src={avatar} alt="avatar" className="w-[40px] height-[40px]"/>
          <p className={`hidden lg:inline ${isOpen ? 'lg:visible' : ''}`}>Paul Garcia</p>
        </div>
